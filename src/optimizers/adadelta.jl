...
function update(self :: AdaDelta, index :: Int, weight :: NDArray,
  ...
  @inplace state.acc .+= (1 - self.opts.rho) * broadcast(*, grad, grad)

  # Compute update using the "old" state.delta_acc
  update = broadcast(*, grad, sqrt(state.delta_acc + self.opts.epsilon)) ./
  ...
  @inplace state.delta_acc .+= (1 - self.opts.rho) * broadcast(*, update, update)
end
